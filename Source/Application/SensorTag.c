...
/******************************************************
 * ENABLE GUIDE DOG REMOTE HERE
 */
#define GUIDE_DOGS_REMOTE 1
...
static uint8_t advertData[] =
{
  ...
  TI_ST_DEVICE_ID,
  TI_ST_KEY_DATA_ID,
  0x00,                                    // Key state - low byte
  0x00									   // Key state - high byte
};

// GAP GATT Attributes
static const uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "Remote";
...
static const uint8_t devInfoModelNumber[] = "Remote";
static const uint8_t devInfoNA[] =          "1.5";
static const uint8_t devInfoFirmwareRev[] = FW_VERSION_STR;
static const uint8_t devInfoMfrName[] =     "Microsoft";
static const uint8_t devInfoHardwareRev[] = "PCB 1.0";
...
#ifndef GUIDE_DOGS_REMOTE
    Board_KEY_1   | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,        /* Button is active low          */
    Board_KEY_2  | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,        /* Button is active low         */
    Board_KEY_3      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,      /* Relay is active high          */
	Board_KEY_4      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
    Board_BUZZER     | PIN_GPIO_OUTPUT_EN | PIN_GPIO_LOW | PIN_PUSHPULL | PIN_DRVSTR_MAX,     /* Buzzer initially off          */
#else
	Board_KEY_1      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_2      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_3      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_4      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_5      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_6      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_7      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_8      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_9      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_10      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_11      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_KEY_12      | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_BOTHEDGES | PIN_HYSTERESIS,
	Board_BUZZER     | PIN_GPIO_OUTPUT_EN | PIN_GPIO_LOW | PIN_PUSHPULL | PIN_DRVSTR_MAX,     /* Buzzer initially off */
...
static void SensorTag_init(void)
{
  ...
  SensorTagBatt_init(); 						  // Battery level
...
static void SensorTag_callback(PIN_Handle handle, PIN_Id pinId)
{
  ...
  case Board_KEY_2:
  case Board_KEY_3:
    SensorTagKeys_processKey(pinId);
    break;
  case Board_KEY_RELAY:
 ...
 * @return  none
 */
void sensorTag_updateAdvertisingData(uint16_t keyStatus)
{
  // Record key state in advertising data
  advertData[KEY_STATE_OFFSET] = LO_UINT16(keyStatus);
  advertData[KEY_STATE_OFFSET + 1] = HI_UINT16(keyStatus);
  GAPRole_SetParameter(GAPROLE_ADVERT_DATA, sizeof(advertData), advertData);
}
